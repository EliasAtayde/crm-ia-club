...
const config: VercelConfig = {
  crons: [
    ...
    // NOTA (deploy Vercel Hobby): schedule original era per-minute
    // (*/1 * * * *), mas o plano Hobby só permite 1 execução/dia por cron.
    // Ajustado para diário, em horários escalonados, até migrar pro plano
    // Pro (onde o schedule per-minute original pode ser restaurado).
    { path: "/api/v1/cron/agent-dispatcher", schedule: "5 0 * * *" },
    { path: "/api/v1/cron/routing-worker", schedule: "10 0 * * *" },
    { path: "/api/v1/cron/event-log-drain", schedule: "15 0 * * *" },
  ],
  functions: {
